...
- **Amendement (multiplicités UML)** : `php-class-diagram` émet parfois des annotations de multiplicité entre guillemets de part et d'autre de la flèche, ex. `A "1" ..> "*" B`. Le Parser les capture optionnellement dans deux champs additifs sur `Relation` (`sourceMultiplicity`, `targetMultiplicity`, tous deux `null` en leur absence) ; elles ne sont jamais requises pour reconnaître une relation. Le Writer les réémet à l'identique de part et d'autre de la flèche pour préserver l'invariant de round-trip (plan §11). Seule la forme entre guillemets est reconnue — une multiplicité non guillemetée serait indiscernable d'un alias supplémentaire et n'est pas supportée.
...
4. **Raffinement** (`ClusterRefiner`) :
    ...
    - **Amendement (misc non borné)** : un split() suivi d'un unique merge() ne garantit pas max-size — un cluster `misc` (ou tout cluster ayant reçu une fusion de petits clusters non connectés via le voisin le plus connecté) n'est produit/agrandi que pendant merge(), strictement après split(), et n'est jamais revérifié ensuite. Le Refiner répète le cycle split→merge jusqu'à `ClusterRefiner::MAX_REFINE_ROUNDS` (5) fois, en s'arrêtant dès qu'aucun cluster ne dépasse max-size, ou dès que l'ensemble des clusters (nom + membres triés) est identique au round précédent. Sans convergence après le nombre de rounds maximal, l'état obtenu est conservé tel quel (comme les autres boucles bornées du projet, ex. `ModularityOptimizer::MAX_PASSES`) et signalé par l'avertissement générique de cluster surdimensionné déjà émis par `SplitCommand`. Ce comportement s'applique de façon générique à tout cluster, pas seulement `misc` par son nom.
      La fusion `mostConnected` elle-même ne pousse jamais sa cible au-delà de max-size : une cible dont la taille dépasserait la limite est exclue des candidats (le petit cluster retombe alors vers `misc` plutôt que de faire déborder un cluster normal) — la boucle ci-dessus reste un filet de sécurité pour `misc` lui-même, pas la seule ligne de défense contre le dépassement. Une composante confirmée non-splittable par la stratégie (mêmes membres exacts) n'est pas re-tentée aux rounds suivants — seul un ensemble de membres différent (après une fusion) redonne une chance à la stratégie. Quand la stratégie est `auto`, seules les décisions du round qui produit l'état final sont conservées pour le rapport `--dry-run` (`AutoClusterer::resetDecisions()`, appelé par le Refiner avant chaque round) : les décisions de rounds intermédiaires, remplacées par une fusion suivante, ne s'accumulent pas dans le rapport.
...
- **SvgRenderer** : si `--render`, invoque `plantuml -charset utf-8 -tsvg` via `symfony/process` sur chaque `.puml` ; erreur claire si le binaire est absent (`--plantuml-bin` pour le chemin).
  **Amendement (perf, lots de fichiers)** : par lots de 50 fichiers (`SvgRenderer`, `$batchSize` configurable au constructeur, non exposé en option CLI) plutôt qu'une invocation par fichier — un run à 100+ fichiers ne démarre plus la JVM PlantUML qu'un nombre de fois divisé par ~50 ; la limite pratique restante est `ARG_MAX` du système, hors d'atteinte à cette échelle. Chaque `.puml` produit toujours son propre `.svg` (dérivation `.puml`→`.svg` appliquée à la liste complète, indépendamment du découpage en lots). En cas d'échec d'un lot, l'erreur rapporte l'ensemble des fichiers du lot fautif et la sortie combinée stdout/stderr de PlantUML (qui nomme déjà le fichier en cause dans son propre message), plutôt qu'une attribution précise par fichier ; le premier lot en échec interrompt immédiatement le rendu, les lots suivants ne sont pas tentés.
