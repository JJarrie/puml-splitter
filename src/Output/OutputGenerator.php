...
use PumlSplitter\Graph\ClusterSlug;
...
final class OutputGenerator
{
    ...
    public function generate(
        ...
        foreach ($partition->clusters as $index => $cluster) {
            $slug = ClusterSlug::unique(ClusterSlug::of($cluster->name), $used);
            $slugs[$index] = $slug;
        ...
        $sharedTypesSlug = $separateHubs === [] ? 'shared_types' : ClusterSlug::unique('shared_types', $used);
